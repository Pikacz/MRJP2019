...
namespace antlr4 {
  ...
  public:
    ...
    /// <summary>
    /// Reset the error handler state for the specified { recognizer}. </summary>
     ...
     * <b>not</b> call { Parser#consume} after a successful recovery.</p>
     *
     * <p>Note that the calling code will not report an error if this method
     * returns successfully. The error strategy implementation is responsible
     * for calling { Parser#notifyErrorListeners} as appropriate.</p>
     *
    ...
    /// { Parser#createErrorNode(ParserRuleContext, Token)} then
    /// { ParserRuleContext#addErrorNode(ErrorNode)} instead of
    /// { Parser#createTerminalNode(ParserRuleContext, Token)}.
    /// </summary>
    /// <param name="recognizer"> the parser instance </param>
    /// <returns> { true} if the parser is currently recovering from a parse
    /// error, otherwise { false} </returns>

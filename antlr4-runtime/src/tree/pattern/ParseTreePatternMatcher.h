...
namespace pattern {
  ...
  /// a <seealso cref="ParseTree"/> with placeholders for the { ID} and { expr}
  /// subtree. Then the <seealso cref="#match"/> routines can compare an actual
  /// <seealso cref="ParseTree"/> from a parse with this pattern. Tag { <ID>} matches
  ...
  /// be { 0}.
  /// <p/>
  /// The <seealso cref="#matches"/> routines return { true} or { false} based
  ...
  public:
    ...
    /// </param>
    /// <exception cref="IllegalArgumentException"> if { start} is { null} or empty. </exception>
    /// <exception cref="IllegalArgumentException"> if { stop} is { null} or empty. </exception>
    virtual void setDelimiters(const std::string &start, const std::string &stop, const std::string &escapeLeft);

    /// <summary>
    /// Does { pattern} matched as rule { patternRuleIndex} match { tree}? </summary>
    ...
    /// { tree} and return a <seealso cref="ParseTreeMatch"/> object that contains the
  ...
  protected:
    ...
    /// { match.}<seealso cref="ParseTreeMatch#labels labels"/>.
    ///

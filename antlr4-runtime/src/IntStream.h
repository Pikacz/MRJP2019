...
namespace antlr4 {
  ...
  public:
    ...
    ///   <li><strong>Ordered lookahead:</strong> The value of { LA(1)} before
    ...
    /// end of the stream (i.e. if { LA(1)==}<seealso cref="#EOF EOF"/> before calling
    /// { consume}). </exception>
    ...
    /// <ul>
    ///   <li>{ i>0}</li>
    ///   <li>{ i==-1} and <seealso cref="#index index()"/> returns a value greater
    ...
    ///   <li>{ LA(i)} refers to a symbol consumed within a marked region
    ...
    /// <seealso cref="#mark mark()"/>. Calls to { release()} must appear in the
    ...
    /// </summary>
    /// <param name="marker"> A marker returned by a call to { mark()}. </param>
    ...
    ///   <li>{ LA(1)} returns <seealso cref="#EOF"/></li>
    /// </ul>
    ///

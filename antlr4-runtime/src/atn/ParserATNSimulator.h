...
namespace atn {
   ...
   *
   * <pre>
   * parser.{ Parser#getInterpreter() getInterpreter()}.{ #setPredictionMode setPredictionMode}{ (}{ PredictionMode#SLL}{ )};
   * parser.{ Parser#setErrorHandler setErrorHandler}(new { BailErrorStrategy}());
   * </pre>
   *
   * <p>
   ...
   * a smaller LL set called <em>s</em>. If <em>s</em> is { {2, 3}}, then SLL
   * parsing will get an error because SLL will pursue alternative 1. If
   * <em>s</em> is { {1, 2}} or { {1, 3}} then both SLL and LL will
   * choose the same alternative because alternative one is the minimum of either
   * set. If <em>s</em> is { {2}} or { {3}} then SLL will get a syntax
   * error. If <em>s</em> is { {1}} then SLL will succeed.</p>
   *
   * <p>
  ...
  protected:
    ...
    /// { configs} which are in a <seealso cref="RuleStopState"/>. If all
    /// configurations in { configs} are already in a rule stop state, this
    /// method simply returns { configs}.
    /// <p/>
    /// When { lookToEndOfRule} is true, this method uses
    /// <seealso cref="ATN#nextTokens"/> for each configuration in { configs} which is
    ...
    /// the configurations from { configs} which are in a rule stop state </returns>
     ...
     * <li>Operator predicates (represented by { SemanticContext.AND} and
    ...
    /// <seealso cref="#addDFAState"/> to ensure the { to} state is present in the
    ...
    /// <seealso cref="#addDFAState"/> for the { to} state.
    /// </summary>
    /// <param name="dfa"> The DFA </param>
    ...
    /// <p/>
    /// If { D} is <seealso cref="#ERROR"/>, this method returns <seealso cref="#ERROR"/> and

...
namespace antlr4 {
  ...
  public:
    ...
    /// parsing, otherwise { false} </returns>
    ...
    /// </summary>
    /// <param name="trimParseTrees"> { true} to trim the capacity of the <seealso cref="ParserRuleContext#children"/>
    /// list to its size after a rule is parsed. </param>
    virtual void setTrimParseTree(bool trimParseTrees);

    /// <returns> { true} if the <seealso cref="ParserRuleContext#children"/> list is trimmed
    ...
    /// </param>
    /// <exception cref="NullPointerException"> if {} listener is { null} </exception>
    ...
    /// { ParserRuleContext#addErrorNode(ErrorNode)} and
    ...
    /// the ATN, otherwise { false}. </returns>
    ...
    /// Get a rule's index (i.e., { RULE_ruleName} field) or INVALID_INDEX if not found.
     ...
     *  returned. I would prefer having { ParserRuleContext#addAnyChild(ParseTree)}
     *  set the parent pointer, but the parent pointer is implementation dependent
     *  and currently there is no setParent() in { TerminalNode} (and can't
    ...
    /** How to create an error node, given a token, associated with a parent.
       ...
       *  returned. I would prefer having { ParserRuleContext#addAnyChild(ParseTree)}
       *  set the parent pointer, but the parent pointer is implementation dependent
       *  and currently there is no setParent() in { ErrorNode} (and can't
       *  add method in Java 1.7 without breaking backward compatibility).
       *
       * @since 4.7
       */
